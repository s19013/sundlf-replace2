...
ブランチ移動などにより、処理内容が変わってしまうため、どのブランチで調査したのか、調査した時のhead の commit id、作成日、更新日を必ず記録する。
各ファイルの先頭に以下を記載する:

```
- ブランチ: <git branch --show-current の結果>
- commit: コミットのid
- 作成日: YYYY-MM-DD HH:ii
- 更新日: YYYY-MM-DD HH:ii
```

ブランチ移動で処理内容が一時的に古くなった場合は、別途ファイルを作成する。
その際はブランチ名に含まれる `/` や `\` などのパス区切り文字の連続を `-` に置換し、
ファイルシステム上安全な名前へ正規化する（例: `feature/memo-api` → `feature-memo-api`）。
元ファイル名に `.branch-<正規化したブランチ名>` を付けたファイル名にし、元ファイルの
冒頭に「<元のブランチ名>では `<実際のブランチ別ファイル名>` を参照」と一行追記する。
正規化後の名前が別の元ブランチ名から作成済みのファイル名と衝突する場合は、元ブランチ名の
SHA-256ハッシュを先頭8文字から一意になるまで末尾へ追加し、参照記述にもその一意化後の名前を使う。
featureブランチがマージ・削除されたら、対応するブランチ別ファイルは元ファイルへ
統合するか削除する。

調査を伴うタスクを完了する際は、`.research-docs/` に記録すべき再利用可能な知見が発生したかどうかを必ず確認し、
該当があれば作業完了報告の前に記録すること。
