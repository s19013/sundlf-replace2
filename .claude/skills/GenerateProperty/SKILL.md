...
1. `mise exec:laravel "php artisan ide-helper:model --nowrite -n"`を実行し、`_ide_helper_models.php`に全モデルの候補を生成する
...
3. `_ide_helper_models.php`に出力された`@property`/`@property-read`の行だけを、対応するモデルクラス宣言の直前に貼り付ける
